                    ...
                    <td className="p-4">
                      <div className="flex items-center gap-4">
                        <div className="w-12 h-12 rounded-lg overflow-hidden shrink-0 border border-gray-800">
                           <img src={project.image.startsWith('/') ? `${(process.env.NEXT_PUBLIC_API_URL || 'https://api.vi-stocks.com').replace(/(https?:\/\/[^/]+)\/api/, "$1")}${project.image}` : project.image} className="w-full h-full object-cover" alt="" />
                        </div>
                        <div>
                      ...
                      <div className="flex flex-col md:flex-row gap-4 items-start">
                         <div className="relative group shrink-0">
                            <div className="w-32 h-32 rounded-2xl bg-gray-900 border-2 border-dashed border-gray-800 overflow-hidden flex items-center justify-center transition-all group-hover:border-blue-500/50">
                               {formData.image ? (
                                  <img src={formData.image.startsWith('/') ? `${(process.env.NEXT_PUBLIC_API_URL || 'https://api.vi-stocks.com').replace(/(https?:\/\/[^/]+)\/api/, "$1")}${formData.image}` : formData.image} className="w-full h-full object-cover" alt="" />
                               ) : (
